...
from mmdet3d.models.losses.centernet_loss import FastFocalLoss, RegLoss, IouLoss, IouRegLoss
...
class MEFormerHead_iou(BaseModule):
    def __init__(self,
                 in_channels,
                 use_ensemble=False,
                 ensemble=None,
        ...
        super(MEFormerHead_iou, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        super(MEFormerHead_iou, self).init_weights()
    ...
    def _loss_single_task(self,
        ...
        #import pdb;pdb.set_trace()
        #iou_reg_loss = torch.tensor(0.0).to(pred_bboxes.device)
        #if self.with_iou_reg:
        #    iou_reg_loss = self.crit_iou_reg(pred_bboxes_flatten, normalized_bbox_targets[isnotnan,:10] )
        #    iou_reg_loss = 0.25 * iou_reg_loss
        #import pdb; pdb.set_trace()
        if self.with_iou :
    ...
    def _dn_loss_single_task(self,
            ...
            if self.with_iou:
                ...
                pred_bboxes_flatten_for_iou = denormal_pred[isnotnan, :10].detach()
                iou_loss = self.crit_iou(pred_ious[isnotnan],pred_bboxes_flatten_for_iou,known_bboxs[isnotnan,:10], num_tgt)
    ...
    def dn_loss_single(self,
                       ...
                       dn_mask_dict):
        loss_cls_tasks, loss_bbox_tasks,loss_iou_tasks = multi_apply(
            self._dn_loss_single_task, pred_bboxes, pred_logits, pred_ious, dn_mask_dict
        )
        return sum(loss_cls_tasks), sum(loss_bbox_tasks), sum(loss_iou_tasks)
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for i, modality in enumerate(preds_dicts[0][0]["modalities"]):
            ...
            last_decoder_index = num_decoder -1
            ...
            for task_id, preds_dict in enumerate(preds_dicts, 0):
                    ...
                    pred_bbox = [preds_dict[0]['center'][i][last_decoder_index], preds_dict[0]['height'][i][last_decoder_index],
                                 preds_dict[0]['dim'][i][last_decoder_index], preds_dict[0]['rot'][i][last_decoder_index]]
                    if 'vel' in preds_dict[0]:
                        pred_bbox.append(preds_dict[0]['vel'][i][last_decoder_index])
                    pred_bbox = torch.cat(pred_bbox, dim=-1)
                    all_pred_bboxes[last_decoder_index].append(pred_bbox)
                    all_pred_logits[last_decoder_index].append(preds_dict[0]['cls_logits'][i][last_decoder_index])
                    if 'iou' in preds_dict[0]:
                        pred_iou = preds_dict[0]['iou'][i][last_decoder_index]
                        all_pred_ious[last_decoder_index].append(pred_iou)
            all_pred_bboxes = [all_pred_bboxes[last_decoder_index]]
            all_pred_logits = [all_pred_logits[last_decoder_index]]
            all_pred_ious = [all_pred_ious[last_decoder_index]]
            #
            loss_cls, loss_bbox , loss_iou = multi_apply(
                self.loss_single, all_pred_bboxes, all_pred_logits,all_pred_ious,
                [gt_bboxes_3d for _ in range(1)],
                [gt_labels_3d for _ in range(1)],
            )
            #import pdb; pdb.set_trace()
            ...
            dn_pred_bboxes, dn_pred_logits, dn_pred_ious = collections.defaultdict(list), collections.defaultdict(list), collections.defaultdict(list)
            dn_mask_dicts = collections.defaultdict(list)
            for task_id, preds_dict in enumerate(preds_dicts, 0):
               
                    pred_bbox = [preds_dict[0]['dn_center'][i][last_decoder_index], preds_dict[0]['dn_height'][i][last_decoder_index],
                                 preds_dict[0]['dn_dim'][i][last_decoder_index], preds_dict[0]['dn_rot'][i][last_decoder_index]]
                    if 'vel' in preds_dict[0]:
                        pred_bbox.append(preds_dict[0]['dn_vel'][i][last_decoder_index])
                    if 'iou' in preds_dict[0]:
                        pred_iou = preds_dict[0]['dn_iou'][i][last_decoder_index]
                        dn_pred_ious[last_decoder_index].append(pred_iou)
                    pred_bbox = torch.cat(pred_bbox, dim=-1)
                    dn_pred_bboxes[last_decoder_index].append(pred_bbox)
                    dn_pred_logits[last_decoder_index].append(preds_dict[0]['dn_cls_logits'][i][last_decoder_index])
                    dn_mask_dicts[last_decoder_index].append(preds_dict[0]['dn_mask_dict'])
            dn_pred_bboxes = [dn_pred_bboxes[last_decoder_index]]
            dn_pred_logits = [dn_pred_logits[last_decoder_index]]
            dn_pred_ious = [dn_pred_ious[last_decoder_index]]
            dn_mask_dicts = [dn_mask_dicts[last_decoder_index]]
            dn_loss_cls, dn_loss_bbox ,dn_loss_iou = multi_apply(
                self.dn_loss_single, dn_pred_bboxes, dn_pred_logits,dn_pred_ious, dn_mask_dicts
            )

            loss_dict[f'dn_loss_cls_{modality}'] = dn_loss_cls[-1]
            loss_dict[f'dn_loss_bbox_{modality}'] = dn_loss_bbox[-1]
            loss_dict[f'dn_loss_iou_{modality}'] = dn_loss_iou[-1]
            
            # num_dec_layer = 0
            # for loss_cls_i, loss_bbox_i  , loss_iou_i in zip(dn_loss_cls[:-1],
            #                                    dn_loss_bbox[:-1],dn_loss_iou[:-1]):
            ...
            #     loss_dict[f'd{num_dec_layer}.dn_loss_iou_{modality}'] = loss_iou_i
...
class MEFormerHead(BaseModule):
    def __init__(self,
        ...
        if train_cfg:
            ...
            self.sampler = build_sampler(sampler_cfg, context=self)
        if failure_pred:
            self.failure_pred = nn.Sequential(
                nn.Linear(self.hidden_dim, 1),
                nn.Sigmoid()
            )
            self.failure_pred_loss = nn.BCELoss().cuda()
        else:
            self.failure_pred = False
    def init_weights(self):
    ...
    def forward_single(self, x, x_img, img_metas):
        ...
        for task_id, task in enumerate(self.task_heads, 0):
            ...
            if self.failure_pred:
                num_layers, bs, num_q, num_c = outs_dec.shape
                failure_pred = self.failure_pred(outs_dec[0][:,:int(num_q*(2/3)),:])
                failure_center = outs['center'][0][:,:int(num_q*(2/3)),:]
                failure_center = failure_center.unsqueeze(2)
                failure_loss_list = []
                for batch in range(bs):
                    if 'point_mask' in img_metas[batch]:
                        if img_metas[batch]['point_mask']:
                            masks_tensor = torch.tensor(img_metas[batch]['point_mask']).cuda()
                            x_check = (failure_center[batch, :, :, 0] >= masks_tensor[:, 0]) & (failure_center[batch, :, :, 0] <= masks_tensor[:, 1])
                            y_check = (failure_center[batch, :, :, 1] >= masks_tensor[:, 2]) & (failure_center[batch, :, :, 1] <= masks_tensor[:, 3])
                            in_mask = x_check & y_check  # Shape: [2, N, M]
                            in_any_mask = in_mask.any(dim=1)  # Shape: [2, N]
                            out_mask = ~in_any_mask
                            failure_loss = self.failure_pred_loss(failure_pred[batch], out_mask.float().unsqueeze(-1))
                            failure_loss_list.append(failure_loss)
                failure_loss_list = sum(failure_loss_list)
            if self.use_ensemble and "fused" in modalities:
                pc_range = torch.Tensor(self.pc_range).to(reference.device)
                outs = self.ensemble(
                    outs_dec,
            ...
            if mask_dict and mask_dict['pad_size'] > 0:
                ...
                outs['dn_mask_dict'] = task_mask_dict
            if self.failure_pred:
                outs['failure_loss'] = failure_loss_list
            ret_dicts.append(outs)
        return ret_dicts
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for i, modality in enumerate(preds_dicts[0][0]["modalities"]):
            ...
            for loss_cls_i, loss_bbox_i in zip(dn_loss_cls[:-1],
                ...
                num_dec_layer += 1
        if self.failure_pred:
            if preds_dicts[0][0]['failure_loss'] != 0:
                loss_dict['failure_loss'] = preds_dicts[0][0]['failure_loss']
            else:
                loss_dict['failure_loss'] = torch.tensor(0.0).cuda()
        return loss_dict
